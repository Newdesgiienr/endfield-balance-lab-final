(() => {
  ...
  // V17 클리어 타임 모델: 총 HP ÷ 평균 DPS 대신 실제 누적 피해 타임라인을 웨이브마다 0초부터 재생합니다.
  // 각 웨이브는 독립적으로 사이클을 초기화하며, 웨이브 사이 전환 시간은 기존 총 15초(3 × 5초)를 유지합니다.
  ...
  function cycleTimeToHp(curve, duration, hp) {
    ...
    // 정확히 한 사이클 피해량의 배수인 경우에도 마지막 공격 시각에서 종료되도록 epsilon을 사용합니다.
  ...
  function clearEstimateWithWaveRows(party, waveRows) {
    const curve = normalizeDamageCurve(party.curve, party.duration, party.totalDamage);
    return simulateWaveTimelineClear(curve, party.duration, waveRows).totalTime;
  }

  const clearEstimate = (party, hpMultiplier = 1) => clearEstimateWithWaveRows(party, baseWaveRows(hpMultiplier));
  ...
  function constraintResultTimeMode(){return window.CONSTRAINT_RESULT_TIME_MODE==='dps'?'dps':'practical';}
  function controlTimeForParty(party,comboState){const ids=Array.isArray(window.CONSTRAINT_SELECTED_IDS)?window.CONSTRAINT_SELECTED_IDS:[];let compositionId='c1';if(window.ControlTimeEngine?.compositionFromConstraintIds)compositionId=window.ControlTimeEngine.compositionFromConstraintIds(ids);else if(comboState?.stageEnhanceEnabled)compositionId=Number(comboState.stageEnhanceLevelDisplay)>=2?'c3':'c2';const calculation=window.ControlTimeEngine?.calculate?.(party.id,compositionId);return{compositionId,seconds:Number(calculation?.total)||0};}
  ...
  function resultBar(root, p, subtitle, metrics, directTarget = '') {
    ...
    bar.innerHTML = `<div class="result-party-id"><div class="result-portraits">${portraitsHtml(p)}</div><div><b>${p.name}</b></div></div>${metrics.map(m => `<div class="result-metric"><span>${m.label}</span><b class="${m.className || ''}">${m.value}</b></div>`).join('')}${action}`;
  }
  ...
  function updateComboConstraint() {
    ...
    results.forEach(({ p, result }) => {
      const controlTime = controlTimeForParty(p, state);
      const displayRemaining = constraintResultTimeMode() === 'practical' ? result.limit - result.clear - controlTime.seconds : result.remaining;
      const root = panel('combo', p), dpsChange = (result.dps / p.dps - 1) * 100, remainingLabel = displayRemaining >= 0 ? '남은 시간' : '초과 시간';
      ...
      $('[data-role="comboTimeMetrics"]', root).innerHTML = [['원본 예상 클리어', seconds(clearEstimate(p)), ''], ['조합 예상 클리어', seconds(result.clear), extra > 0 ? 'negative' : ''], ['적용 제한시간', `${result.limit}초`, ''], [remainingLabel, seconds(Math.abs(displayRemaining)), displayRemaining < 0 ? 'negative' : 'positive']].map(([l, v, c]) => `<div><span>${l}</span><b class="${c}">${v}</b></div>`).join('');
      ...
      parts.push(displayRemaining < 0 ? `현재 선택한 시간 기준에서 적용 제한시간을 ${num(Math.abs(displayRemaining))}초 초과합니다.` : `현재 선택한 시간 기준에서 ${num(displayRemaining)}초의 예상 여유가 남습니다.`);
